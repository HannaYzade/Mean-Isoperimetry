...
void find_path(int v, int _k, int w, int d, int _rho,  bool in_par_cluster, bool in_child_cluster, bool is_par_dp) {
    if(!is_par_dp) { //dp
        if (w == 0)
            ans[v] = 0;
        else 
        ...
        if ((int)child[v].size() == 0)
            return;
        find_path (v, _k, w, d, _rho, false, in_child_cluster, true);
    }
    else { //par_dp
        ...
        for (int i = (int)child[v].size() - 1; i >= 0; i --) {
            ...
            find_path(u, _par.k, _par.w, _par.d, _par.rho, in_child_cluster, _par.b, false);
            _k -= _par.k;
...
int main() {
    ...
    for (int i = 1; i < n; i ++) {
        ...
        cin >> _par >> _w;
        par[i] = {_par, _w};
        child [_par].push_back(i), c[_par].push_back(_w);
    }

    //solve --> filling dp / par_dp / par_dp_path
    dfs (0);
    ...
    find_path(0, root.k, root.w, root.d, root.rho, false, root.b, false); //function inputs: _v, _k, _w, _d, _rho, in_par_cluster, in_child_cluster, is_par_dp
